...
| NBT | `gtlcoreGraphJob` / `engineKind=graph` / schemaVersion=4，与 legacy job 分开 |
...
返还物读取，因此它的耗时不等于“复制库存”的耗时。
混合输入与跨槽组合展开、数量合并、指纹编码、目录索引、图构建、求解与计划处理在后台执行。

`CandidateCapture` 在主线程捕获每槽合法候选的确定值、倍率、配置物标记与返还物，
`CapturedPattern` 只保留不可变值；后台不调用附属的 `isValid`、`getRemainingKey` 或读取库存。
通过有上限的组合计数与候选首次出现序号发现依赖，保留原先前 256 个变体的范围及顺序，
不必为了发现依赖而在主线程生成每个变体，也不会额外递归未入选的模糊候选。
原生精确处理样板仍须验证声明输入，但每槽可以一次捕获；自定义输入与附属包装保留通用验证。

目录至少含 512 个变体且线程数大于 1 时，展开和指纹编码使用现有规划池分批并行。
每批最多 64 个样板、256 个变体，每波任务数不超过配置线程数；按原始样板顺序合并，
保留提供者优先级与相同指纹的去重语义。小目录直接在单个后台工作片执行。
各任务共用请求的取消、搜索及内存预算；完整准备成功后才发布编译缓存。
这减少主线程工作量，但样板发现、谓词回调与库存采集仍受主线程调度影响，不能保证完全不受 TPS 影响。
...
循环和保种路径使用最多 32 个未派发批次的前瞻窗口。提前派发时保护前序步骤的材料需求和数量峰值，
允许部分实物回料启动下游；所有在途产物继续记账，最终结清仍等待完整回料。
重复程序保持压缩，窗口与剩余游标一起保存；schema 1–3 可读取，不明转移保持暂停。
...
- 多配方循环支持有限窗口内的安全流水线，不保证全局最优调度；窗口之外的步骤仍需等待前序推进。
  已承诺的物理批次不能由重规划撤销或重发。
